...
public class  ProductService {
	...
	//单个查找
	public List<Product> findPageBeanProductListbykey(int index, int currentCount, String key) {
		ProductDao dao = new ProductDao();
		List<Product> productList = null;
		try {
			productList = dao.findPageBeanProduct(index,currentCount,key);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public int findAllProductByKey(String key) {
		ProductDao dao = new ProductDao();
		int totalCount = 0;
		try {
			totalCount = dao.findAllProductByKey(key);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
