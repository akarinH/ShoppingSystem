			...
			<ol class="breadcrumb">
				...
				<%-- //////////////////////////////////--%>
					<form id="sort-form" class="form-inline"
						action="${pageContext.request.contextPath}/product?method=product_list&cid=${cid}"
						method="post">
						...
						<div class="form-group"> <label for="sort">排序顺序：</label>
							<select id="sort" name="sort" class="form-control">
								<option value="sort">综合</option>
					...
					<%-- ///////////////////////--%>
						...
						<c:forEach items="${pageBean.list }" var="product">
							<div class="col-md-2" style="height: 250px; text-align: center;">
								<a
									href="${pageContext.request.contextPath }/product?method=product_info&pid=${product.pid}&cid=${cid}&currentPage=${pageBean.currentPage}">
			...
			<div style="width: 380px; margin: 0 auto; margin-top: 50px; text-align: center;">
				<ul class="pagination" style="margin-top: 10px;">
					...
					<c:forEach begin="1" end="${pageBean.totalPage }" varStatus="page">
						...
						<c:if test="${pageBean.currentPage!=page.count }">
							<li><a
									href="${pageContext.request.contextPath }/product?method=product_list&cid=${cid}&sort=${sort}&currentPage=${page.count}">${page.count
									}</a></li>
						</c:if>
					</c:forEach>
					...
					<c:if test="${pageBean.currentPage!=pageBean.totalPage }">
						<li>
							<a href="${pageContext.request.contextPath }/product?method=product_list&cid=${cid}&sort=${sort}&currentPage=${pageBean.currentPage+1}"
								aria-label="Next">
								<span aria-hidden="true">&raquo;</span>
							</a>
						</li>
				...
				<div style="overflow: hidden;">
					<ul style="list-style: none;">
						<c:forEach items="${historyProductList }" var="pro">
							<li
								style="width: 216px; height: 216; float: left; margin: 0 8px 0 0; padding: 0 18px 15px; text-align: center;">
								<img src="${pageContext.request.contextPath }/${pro.pimage}" width="130px"
									height="130px" />
								<p>
