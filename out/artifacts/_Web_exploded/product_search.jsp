...
<body>
	...
	<div style="width: 380px; margin: 0 auto; margin-top: 50px; text-align: center;">
		<ul class="pagination" style="margin-top: 10px;">
			...
			<c:if test="${pageBean.currentPage!=1 }">
				<li >
					<a href="${pageContext.request.contextPath }/product?method=product_search&currentPage=${pageBean.currentPage-1}"
					aria-label="Previous">
						<span aria-hidden="true">&laquo;</span>
					</a>
				</li>
			...
			<c:forEach begin="1" end="${pageBean.totalPage }" varStatus="page">
				...
				<c:if test="${pageBean.currentPage!=page.count }">
					<li><a href="${pageContext.request.contextPath }/product?method=product_search&currentPage=${page.count}">${page.count }</a></li>
				</c:if>
			</c:forEach>
			...
			<c:if test="${pageBean.currentPage!=pageBean.totalPage }">
				<li >
					<a href="${pageContext.request.contextPath }/product?method=product_search&currentPage=${pageBean.currentPage+1}" aria-label="Next">
						<span aria-hidden="true">&raquo;</span>
					</a>
				 </li>
			</c:if>
